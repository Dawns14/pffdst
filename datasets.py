...
def distribute_clients_categorical(x, p, clients=400, beta=0.1):
    ...
    unique, counts = torch.Tensor(x.targets.tolist() if torch.is_tensor(x.targets) else x.targets).unique(return_counts=True)
...
def distribute_clients_dirichlet(train, test, clients=400, batch_size=32, beta=0.1, rng=None):
    ...
    unique = torch.Tensor(train.targets.tolist() if torch.is_tensor(train.targets) else train.targets).unique()
...
def get_mnist_or_cifar10(dataset='mnist', mode='dirichlet', path=None, clients=400,
    ...
    if dataset not in ('mnist', 'fashion-mnist', 'cifar10', 'cifar100'):
...
def get_fashion_mnist(*args, **kwargs):
    return get_mnist_or_cifar10('fashion-mnist', *args, **kwargs)
